...
models_map = {
    ...
    "states": ("WorkflowState", "WorkflowStateResponse", "CatalogBase", "State")
}
...
for module, (model, schema_resp, schema_base, name) in models_map.items():
    with open(f"api-pqrsf/routers/admin/{module}.py", "w", encoding="utf-8") as f:
        f.write(template.replace("{model}", model).replace("{schema_resp}", schema_resp).replace("{schema_base}", schema_base).replace("{name}", name))
...
for m in complex_modules:
    with open(f"api-pqrsf/routers/admin/{m}.py", "w", encoding="utf-8") as f:
