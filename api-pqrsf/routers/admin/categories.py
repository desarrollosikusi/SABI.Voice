...
def get_all(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    return db.query(models.CauseCategory).offset(skip).limit(limit).all()
...
def create(item: schemas.CatalogBase, db: Session = Depends(get_db), current_user: models.User = Depends(auth.get_current_user)):
    db_item = models.CauseCategory(**item.model_dump())
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
    return db_item
...
def update(item_id: int, item: schemas.CatalogBase, db: Session = Depends(get_db), current_user: models.User = Depends(auth.get_current_user)):
    db_item = db.query(models.CauseCategory).filter(models.CauseCategory.id == item_id).first()
    if not db_item:
