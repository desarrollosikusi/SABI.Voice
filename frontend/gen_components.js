...
const componentsDir = path.join('frontend', 'src', 'app', 'dashboard', 'pqrsf', '[id]', 'components');
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ flex: 1, height: 6, backgroundColor: 'var(--surface-border)', borderRadius: 4, overflow: 'hidden' }}>
              <div style={{ width: \\%\, height: '100%', backgroundColor: (clasificacion.nivel_confianza || 0) > 0.8 ? '#10B981' : '#F59E0B' }} />
            </div>
    ...
    <Card>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 32 }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <InfoRow label="Nombre Completo" value={contact ? \\ \\ : data?.correo} />
...
export default function AttachmentsPanel({ data }: { data: any }) {
  ...
  const handleDownload = (att: any) => {
    window.open(\\/pqrsf/\/attachments/\/download\, '_blank');
  };
    ...
    <Card>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
        ...
        {events.map((ev, i) => {
          ...
          return (
            <div key={\\-\\} style={{ display: 'flex', gap: 16, position: 'relative', paddingBottom: 24 }}>
              {/* Line */}
              {i !== events.length - 1 && (
...
export default function OperationalPanel({ data, catalogs, onSave }: { data: any, catalogs: any, onSave: (payload: any) => Promise<void> }) {
  ...
  return (
    <Card style={{ position: 'sticky', top: 24, padding: 24 }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div>
          ...
          <select name="estado_id" value={form.estado_id} onChange={handleChange} className="input-field" style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid var(--surface-border)' }}>
...
for (const [name, content] of Object.entries(files)) {
    fs.writeFileSync(path.join(componentsDir, name), content, 'utf8');
}
